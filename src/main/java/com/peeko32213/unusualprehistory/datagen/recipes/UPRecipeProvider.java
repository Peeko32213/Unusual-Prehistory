...
public abstract class UPRecipeProvider extends RecipeProvider {


    public UPRecipeProvider(PackOutput pOutput) {
        super(pOutput);
    }
    ...
    private void oreSmeltingRecipe(ItemLike result, List<ItemLike> ingredients, float xp, String group, Consumer<FinishedRecipe> consumer) {
        for(ItemLike ingredient : ingredients) {
            smeltingRecipe(result, ingredient, xp, 1).group(group).save(consumer, new ResourceLocation(UnusualPrehistory.MODID, "smelt_" + ForgeRegistries.ITEMS.getKey(ingredient.asItem()).getPath()));
        }
    }
    ...
    private void oreBlastingRecipe(ItemLike result, List<ItemLike> ingredients, float xp, String group, Consumer<FinishedRecipe> consumer) {
        for(ItemLike ingredient : ingredients) {
            blastingRecipe(result, ingredient, xp, 1).group(group).save(consumer, new ResourceLocation(UnusualPrehistory.MODID, "blast_" + ForgeRegistries.ITEMS.getKey(ingredient.asItem()).getPath()));
        }
    }
