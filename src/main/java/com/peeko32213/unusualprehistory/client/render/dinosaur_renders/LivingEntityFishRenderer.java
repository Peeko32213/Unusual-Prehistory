...
import com.peeko32213.unusualprehistory.common.entity.ScaumenaciaEntity;
...
public class LivingEntityFishRenderer<T extends LivingEntity & GeoAnimatable> extends GeoEntityRenderer<T> {
    ...
    protected void applyRotations(T animatable, PoseStack poseStack, float ageInTicks, float rotationYaw, float partialTick) {
        ...
        if(animatable instanceof StethacanthusEntity stethacanthus) {
            if(stethacanthus.isFromBook()) return;
        }
        if(animatable instanceof ScaumenaciaEntity scaumenacia) {
