 ...
 public class OtarocyonEntity extends TamablePrehistoricEntity implements ICustomFollower {
     ...
     protected void registerGoals() {
         ...
         this.goalSelector.addGoal(6, new OtarocyonEntity.FoxPounceGoal());
         this.goalSelector.addGoal(1, new OtarocyonEntity.FaceplantGoal());
     ...
     protected <E extends OtarocyonEntity> PlayState attackController(final software.bernie.geckolib.core.animation.AnimationState<E> event) {
     ...
     public class FoxPounceGoal extends JumpGoal {
         ...
         public boolean canUse() {
             if (!OtarocyonEntity.this.isFullyCrouched()) {
                 return false;
             } else {
                 LivingEntity livingentity = OtarocyonEntity.this.getTarget();
                 if (livingentity != null && livingentity.isAlive()) {
                     if (livingentity.getMotionDirection() != livingentity.getDirection()) {
                         return false;
                     } else {
                         boolean flag = OtarocyonEntity.isPathClear(OtarocyonEntity.this, livingentity);
                         if (!flag) {
                             OtarocyonEntity.this.getNavigation().createPath(livingentity, 0);
                             OtarocyonEntity.this.setIsCrouching(false);
                             OtarocyonEntity.this.setIsInterested(false);
                         }
         ...
         public boolean canContinueToUse() {
             LivingEntity livingentity = OtarocyonEntity.this.getTarget();
             if (livingentity != null && livingentity.isAlive()) {
                 double d0 = OtarocyonEntity.this.getDeltaMovement().y;
                 return (!(d0 * d0 < (double)0.05F) || !(Math.abs(OtarocyonEntity.this.getXRot()) < 15.0F) || !OtarocyonEntity.this.onGround()) && !OtarocyonEntity.this.isFaceplanted();
             } else {
                 return false;
             }
         }
         ...
         public void start() {
             OtarocyonEntity.this.setJumping(true);
             OtarocyonEntity.this.setIsPouncing(true);
             OtarocyonEntity.this.setIsInterested(false);
             LivingEntity livingentity = OtarocyonEntity.this.getTarget();
             if (livingentity != null) {
                 OtarocyonEntity.this.getLookControl().setLookAt(livingentity, 60.0F, 30.0F);
                 Vec3 vec3 = (new Vec3(livingentity.getX() - OtarocyonEntity.this.getX(), livingentity.getY() - OtarocyonEntity.this.getY(), livingentity.getZ() - OtarocyonEntity.this.getZ())).normalize();
                 OtarocyonEntity.this.setDeltaMovement(OtarocyonEntity.this.getDeltaMovement().add(vec3.x * 0.8D, 0.9D, vec3.z * 0.8D));
             }

             OtarocyonEntity.this.getNavigation().stop();
         }


         public void stop() {
             OtarocyonEntity.this.setIsCrouching(false);
             OtarocyonEntity.this.crouchAmount = 0.0F;
             OtarocyonEntity.this.crouchAmountO = 0.0F;
             OtarocyonEntity.this.setIsInterested(false);
             OtarocyonEntity.this.setIsPouncing(false);
         }
         ...
         public void tick() {
             ...
             if (!OtarocyonEntity.this.isFaceplanted()) {
                 Vec3 vec3 = OtarocyonEntity.this.getDeltaMovement();
                 if (vec3.y * vec3.y < (double)0.03F && OtarocyonEntity.this.getXRot() != 0.0F) {
                     OtarocyonEntity.this.setXRot(Mth.rotLerp(0.2F, OtarocyonEntity.this.getXRot(), 0.0F));
                 } else {
                     double d0 = vec3.horizontalDistance();
                     double d1 = Math.signum(-vec3.y) * Math.acos(d0 / vec3.length()) * (double)(180F / (float)Math.PI);
                     OtarocyonEntity.this.setXRot((float)d1);
                 }
             }

             if (livingentity != null && OtarocyonEntity.this.distanceTo(livingentity) <= 2.0F) {
                 OtarocyonEntity.this.doHurtTarget(livingentity);
             } else if (OtarocyonEntity.this.getXRot() > 0.0F && OtarocyonEntity.this.onGround() && (float) OtarocyonEntity.this.getDeltaMovement().y != 0.0F && OtarocyonEntity.this.level().getBlockState(OtarocyonEntity.this.blockPosition()).is(Blocks.SNOW)) {
                 OtarocyonEntity.this.setXRot(60.0F);
                 OtarocyonEntity.this.setTarget((LivingEntity)null);
                 OtarocyonEntity.this.setFaceplanted(true);
             }
     ...
     class FaceplantGoal extends Goal {
         ...
         public boolean canUse() {
             return OtarocyonEntity.this.isFaceplanted();
         }
         ...
         public void stop() {
             OtarocyonEntity.this.setFaceplanted(false);
         }
     ...
     public static boolean isPathClear(OtarocyonEntity pFox, LivingEntity pLivingEntity) {
