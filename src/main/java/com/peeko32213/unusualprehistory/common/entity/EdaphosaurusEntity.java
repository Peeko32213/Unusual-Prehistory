...
public class EdaphosaurusEntity extends BaseDinosaurAnimalEntity {
    ...
    public EdaphosaurusEntity(EntityType<? extends Animal> entityType, Level level) {
    ...
    protected <E extends EdaphosaurusEntity> PlayState Controller(final software.bernie.geckolib.core.animation.AnimationState<E> event) {
        ...
        if (this.isInWater()) {
            ...
            event.setAndContinue(EDAPHO_IDLE);
            event.getController().setAnimationSpeed(1.0F);
        }
        return PlayState.CONTINUE;
    }
