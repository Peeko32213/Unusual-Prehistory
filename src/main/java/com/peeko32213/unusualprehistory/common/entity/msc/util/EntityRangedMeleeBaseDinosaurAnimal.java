...
public abstract class EntityRangedMeleeBaseDinosaurAnimal extends Animal implements IAnimatable, NeutralMob {
    private AnimationFactory factory = GeckoLibUtil.createFactory(this);
    private static final EntityDataAccessor<Boolean> HUNGRY = SynchedEntityData.defineId(EntityRangedMeleeBaseDinosaurAnimal.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> TIME_TILL_HUNGRY = SynchedEntityData.defineId(EntityRangedMeleeBaseDinosaurAnimal.class, EntityDataSerializers.INT);

    private static final EntityDataAccessor<Boolean> SADDLED = SynchedEntityData.defineId(EntityRangedMeleeBaseDinosaurAnimal.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> PASSIVE = SynchedEntityData.defineId(EntityRangedMeleeBaseDinosaurAnimal.class, EntityDataSerializers.INT);

    private static final EntityDataAccessor<Integer> ANGER_TIME = SynchedEntityData.defineId(EntityRangedMeleeBaseDinosaurAnimal.class, EntityDataSerializers.INT);
    public static final EntityDataAccessor<Integer> STATE = SynchedEntityData.defineId(EntityRangedMeleeBaseDinosaurAnimal.class, EntityDataSerializers.INT);
    private static final UniformInt ANGER_TIME_RANGE = TimeUtil.rangeOfSeconds(20, 39);
    private UUID targetUuid;
    private BlockPos lightBlockPos = null;
    int lastTimeSinceHungry;

    protected EntityRangedMeleeBaseDinosaurAnimal(EntityType<? extends Animal> entityType, Level level) {
        ...
        if(hasTargets()) {
            this.targetSelector.addGoal(1, new NearestAttackableTargetGoal<>(this, LivingEntity.class, 10, false, false, entity -> entity.getType().is(getTargetTag())) {
                        ...
                        public boolean canUse() {
                            return ((EntityRangedMeleeBaseDinosaurAnimal) this.mob).isHungry() && super.canUse();
                        }
                    }
            );
        }
   ...
   public AnimationFactory getFactory() {
    ...
    protected PathNavigation createNavigation(Level level) {
        if(hasCustomNavigation()){
            return new EntityRangedMeleeBaseDinosaurAnimal.DinoCustomNavigation(this, level);
        }
    ...
    static class DinoCustomNavigation extends GroundPathNavigation {
        ...
        protected PathFinder createPathFinder(int p_33382_) {
            this.nodeEvaluator = new EntityRangedMeleeBaseDinosaurAnimal.CustomNodeEvaluator();
