...
public class UPBoat extends Boat implements HasUPBoatType {
    private static final EntityDataAccessor<String> BOAT_TYPE = SynchedEntityData.defineId(UPBoat.class, EntityDataSerializers.STRING);

    public UPBoat(EntityType<? extends Boat> type, Level level) {
        super(type, level);
    }

    public UPBoat(Level level, ResourceLocation type, double x, double y, double z) {
        this(UPEntities.BOAT.get(), level);
        this.setType(type);
        this.setPos(x, y, z);
    ...
    public UPBoat(PlayMessages.SpawnEntity spawnEntity, Level level) {
        this(UPEntities.BOAT.get(), level);
    }
    ...
    protected void readAdditionalSaveData(CompoundTag compound) {
        if (compound.contains("Type", Tag.TAG_STRING)) {
            ...
            if (UPBoatTypes.getType(name) != null) this.setType(name);
            else this.setType(UPBoatTypes.UNDEFINED_BOAT_LOCATION);
        } else {
        ...
        if (!this.isPassenger()) {
            if (onGroundIn) {
                if (this.fallDistance > 3.0F) {
                    if (this.status != UPBoat.Status.ON_LAND) {
