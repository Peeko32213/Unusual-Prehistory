...
public class AnurognathusEntity extends AgeableMob implements GeoEntity, NeutralMob, IBookEntity, IVariantEntity {
    ...
    private static final EntityDataAccessor<Boolean> FROM_BOOK = SynchedEntityData.defineId(AnurognathusEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public AnurognathusEntity(EntityType<? extends AgeableMob> entityType, Level level) {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(1, new NearestAttackableTargetGoal<>(this, LivingEntity.class, 10, false, false, entity -> entity.getType().is(UPEntityTypeTags.ANURO_TARGETS)));
    }
    ...
    protected <E extends AnurognathusEntity> PlayState attackController(final software.bernie.geckolib.core.animation.AnimationState<E> event) {
    ...
    private class AIFlyIdle extends Goal {
        ...
        public boolean canUse() {
            if (AnurognathusEntity.this.isVehicle() ||  (AnurognathusEntity.this.getTarget() != null && AnurognathusEntity.this.getTarget().isAlive()) || AnurognathusEntity.this.isPassenger()) {
                return false;
            } else {
                if (AnurognathusEntity.this.getRandom().nextInt(45) != 0 && !AnurognathusEntity.this.isFlying()) {
                    return false;
                }
                if (AnurognathusEntity.this.onGround()) {
                    this.flightTarget = random.nextBoolean();
                } else {
                    this.flightTarget = random.nextInt(5) > 0 && AnurognathusEntity.this.timeFlying < 200;
                }
        ...
        public void tick() {
            if (flightTarget) {
                AnurognathusEntity.this.getMoveControl().setWantedPosition(this.x, this.y, this.z, 1F);
            } else {
                AnurognathusEntity.this.getNavigation().moveTo(this.x, this.y, this.z, 1F);
            }
            if (!flightTarget && isFlying() && AnurognathusEntity.this.onGround()) {
                AnurognathusEntity.this.setFlying(false);
            }
            if (isFlying() && AnurognathusEntity.this.onGround() && AnurognathusEntity.this.timeFlying > 10) {
                AnurognathusEntity.this.setFlying(false);
            }
        }
        ...
        protected Vec3 getPosition() {
            Vec3 vector3d = AnurognathusEntity.this.position();

            if(AnurognathusEntity.this.isOverWaterOrVoid()){
                flightTarget = true;
            }
            if (flightTarget) {
                if (AnurognathusEntity.this.timeFlying < 200 || AnurognathusEntity.this.isOverWaterOrVoid()) {
                    return AnurognathusEntity.this.getBlockInViewAway(vector3d, 0);
                } else {
                    return AnurognathusEntity.this.getBlockGrounding(vector3d);
                }
            } else {
                return LandRandomPos.getPos(AnurognathusEntity.this, 10, 7);
            }
        }

        public boolean canContinueToUse() {
            if (flightTarget) {
                return AnurognathusEntity.this.isFlying() && AnurognathusEntity.this.distanceToSqr(x, y, z) > 5F;
            } else {
                return (!AnurognathusEntity.this.getNavigation().isDone()) && !AnurognathusEntity.this.isVehicle();
            }
        }

        public void start() {
            if (flightTarget) {
                AnurognathusEntity.this.setFlying(true);
                AnurognathusEntity.this.getMoveControl().setWantedPosition(x, y, z, 1F);
            } else {
                AnurognathusEntity.this.getNavigation().moveTo(this.x, this.y, this.z, 1F);
            }
        }
        ...
        public void stop() {
            AnurognathusEntity.this.getNavigation().stop();
            x = 0;
            y = 0;
            z = 0;
            super.stop();
