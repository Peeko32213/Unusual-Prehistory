...
public class HynerpetonEntity extends BaseDinosaurAnimalEntity implements SemiAquatic, GeoEntity {
    ...
    private static final EntityDataAccessor<Boolean> IGNITED = SynchedEntityData.defineId(HynerpetonEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public @NotNull InteractionResult mobInteract(Player pPlayer, @NotNull InteractionHand pHand) {
        ...
        if (itemstack.is(UPTags.HYNERPETON_IGNITERS)) {
            ...
            if (!this.level().isClientSide) {
                this.setIsIgnited(true);
                this.kill();
                this.playSound(SoundEvents.TNT_PRIMED, 1.0F, 1.25F);
    ...
    private void explodeHyperneton() {
        if (!this.level().isClientSide) {
            int EXPLOSION_RADIUS = 1;
            this.level().explode(this, this.getX(), this.getY(), this.getZ(), (float) EXPLOSION_RADIUS, Level.ExplosionInteraction.MOB);
            this.discard();
    ...
    public void addAdditionalSaveData(CompoundTag compound) {
        ...
        compound.putBoolean("ignite", this.isIgnited());
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        setIsIgnited(compound.getBoolean("ignite"));
    }
    ...
    protected <E extends HynerpetonEntity> PlayState Controller(final software.bernie.geckolib.core.animation.AnimationState<E> event) {
        ...
        if(this.isIgnited() && (this.dead || this.getHealth() <= 0.0 || this.isDeadOrDying())){
    ...
    public void setIsIgnited(boolean ignite) {
        this.entityData.set(IGNITED, ignite);
    }
