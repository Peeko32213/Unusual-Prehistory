package com.peeko32213.unusualprehistory.common.entity.custom.ai.goal.boids;
...
public class BoidFishSchoolingGoal extends Goal {
    ...
    public BoidFishSchoolingGoal(BoidFishPrehistoricEntity mob, float separationInfluence, float separationRange, float alignmentInfluence, float cohesionInfluence) {
        this.mob = mob;
    ...
    public Vec3 separation() {
        ...
        for (BoidFishPrehistoricEntity nearbyMob : mob.ownSchool) {
        ...
        if (mob.isFollower()) {
            for (BoidFishPrehistoricEntity nearbyMob : mob.leader.ownSchool) {
                if ((nearbyMob.position().subtract(mob.position()).length()) < separationRange && !nearbyMob.isDeadOrDying()) {
                    c = c.subtract(nearbyMob.position().subtract(mob.position()));
                }
            }
