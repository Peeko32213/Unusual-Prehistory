...
public class EntityPsilopterus extends EntityBaseDinosaurAnimal implements PackHunter {

    private static final EntityDataAccessor<Integer> ANIMATION_STATE = SynchedEntityData.defineId(EntityPsilopterus.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> COMBAT_STATE = SynchedEntityData.defineId(EntityPsilopterus.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> ENTITY_STATE = SynchedEntityData.defineId(EntityPsilopterus.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> DOMINATE = SynchedEntityData.defineId(EntityPsilopterus.class, EntityDataSerializers.BOOLEAN);
    ...
    private EntityPsilopterus priorPackMember;
    private EntityPsilopterus afterPackMember;
    public EntityPsilopterus(EntityType<? extends Animal> entityType, Level level) {
        ...
        if (target != null && target.isAlive() && !(target instanceof Player player && player.isCreative())) {
            if (this.isDominate()) {
                ...
                while (leader.getAfterPackMember() != null) {
                    ...
                    if(!((EntityPsilopterus) leader).isAlliedTo(target)){
                        ((EntityPsilopterus) leader).setTarget(target);
                    }
                }
            }
    ...
    public void setPriorPackMember(PackHunter animal) {
        this.priorPackMember = (EntityPsilopterus) animal;
    }

    @Override
    public void setAfterPackMember(PackHunter animal) {
        this.afterPackMember = (EntityPsilopterus) animal;
    }
    ...
    static class PsiloMeleeAttackGoal extends Goal {
        ...
        protected final EntityPsilopterus mob;
        ...
        public PsiloMeleeAttackGoal(EntityPsilopterus p_i1636_1_, double p_i1636_2_, boolean p_i1636_4_) {
            this.mob = p_i1636_1_;
    ...
    protected <E extends EntityPsilopterus> PlayState Controller(final software.bernie.geckolib.core.animation.AnimationState<E> event) {
        if(this.isFromBook()){
