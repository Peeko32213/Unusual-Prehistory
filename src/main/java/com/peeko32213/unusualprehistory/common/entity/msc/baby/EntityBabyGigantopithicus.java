...
public class EntityBabyGigantopithicus extends EntityTameableBaseDinosaurAnimal implements IAnimatable {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (this.getDeltaMovement().horizontalDistanceSqr() > 1.0E-6 && !this.isInWater() && !this.isSwimming()) {
            {
                event.getController().setAnimation(new AnimationBuilder().loop("animation.baby_gigantopithicus.walk"));
                return PlayState.CONTINUE;

            }
        }
        if (this.isInWater()) {
            event.getController().setAnimation(new AnimationBuilder().loop("animation.baby_gigantopithicus.swim"));
            event.getController().setAnimationSpeed(1.0F);
            return PlayState.CONTINUE;
        }
        else {
            event.getController().setAnimation(new AnimationBuilder().loop("animation.baby_gigantopithicus.idle"));
        }
        return PlayState.CONTINUE;
    }
    ...
    public void registerControllers(AnimationData data) {
        ...
        AnimationController<EntityBabyGigantopithicus> controller = new AnimationController<>(this, "controller", 5, this::predicate);
