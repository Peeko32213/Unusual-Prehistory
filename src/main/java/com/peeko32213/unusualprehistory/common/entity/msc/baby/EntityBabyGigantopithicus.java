...
public class EntityBabyGigantopithicus extends EntityTameableBaseDinosaurAnimal{
    ...
    private int age;
    private static final RawAnimation BABY_WALK = RawAnimation.begin().thenLoop("animation.baby_gigantopithicus.walk");
    private static final RawAnimation BABY_IDLE = RawAnimation.begin().thenLoop("animation.baby_gigantopithicus.idle");
    private static final RawAnimation BABY_SWIM = RawAnimation.begin().thenLoop("animation.baby_gigantopithicus.swim");
    public EntityBabyGigantopithicus(EntityType<? extends EntityTameableBaseDinosaurAnimal> entityType, Level level) {
    ...
    public boolean isFood(ItemStack stack) {
        return EntityBabyGigantopithicus.FOOD_ITEMS.test(stack);
    }
    ...
    private void growUp() {
        if (this.level() instanceof ServerLevel server) {
            EntityGigantopithicus frog = UPEntities.GIGANTOPITHICUS.get().create(this.level());
            if (frog == null) return;
    ...
    protected <E extends EntityBabyGigantopithicus> PlayState Controller(final software.bernie.geckolib.core.animation.AnimationState<E> event) {
        if (this.isInWater()) {
