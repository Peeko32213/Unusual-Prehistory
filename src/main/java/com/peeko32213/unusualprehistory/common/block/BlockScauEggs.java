...
public class BlockScauEggs extends Block {
    protected static final VoxelShape SHAPE = Block.box(0, 0, 0, 16, 1.5, 16);

    public BlockScauEggs(Properties properties) {
        super(properties);
    }
    ...
    private void createTadpole(ServerLevel level, BlockPos pos, Random random) {
        int i = random.nextInt(5, 8);

        for (int index = 1; index <= i; ++index) {
            EntityScaumenacia tadpole = UPEntities.SCAU.get().create(level);
            if (tadpole != null) {
