...
public class BlockBeelzeEggs extends Block {
    protected static final VoxelShape SHAPE = Block.box(0, 0, 0, 16, 1.5, 16);

    public BlockBeelzeEggs(Properties properties) {
        super(properties);
    }
    ...
    private void createTadpole(ServerLevel level, BlockPos pos, Random random) {
        ...
        for (int index = 1; index <= i; ++index) {
            EntityBeelzebufoTadpole tadpole = UPEntities.BEELZE_TADPOLE.get().create(level);
            if (tadpole != null) {
